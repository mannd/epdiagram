...
- Rotated images may overlap the left margin and slightly and thus the edges of the image may not be accessible.  This only practically affects larger rotations (more than just a few degrees).  This is a limitation of the mehtod of rotation.  If your image must be rotated a large amount, it is recommended you rotate it in an external program (for example, using the editor functions of the Photos app), and load the rotated image into your diagram.
* Next version plans
