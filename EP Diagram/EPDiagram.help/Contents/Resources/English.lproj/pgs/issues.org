...
- If you are used to the iPad version of *EP Diagram*, there are some differences and limitations relating to porting the app to the Mac.  These include:
  ...
  - It is sometimes necessary to grant permission to access folders in the Mac.  If so, an open folder dialog will appear requesting the permission. 
- Drag and drop works for image files (e.g. JPG files) but not for PDF or diagram files.  
...
- Attempting to switch diagrams by two-finger clicking on the app icon in the dock and selecting another diagram doesn't work.  This is problem with Mac Catalyst which we hope will be fixed in a later version of macOS.
