...
** Diagrams
*** Welcome screen
The initial screen on starting the app provides shortcuts to create a new diagram, open a previously created diagram, adjust app settings, or get help.

#+CAPTION: Welcome screen
[[../gfx/welcome_screen.png]]

*** Creating and opening diagrams
EP Diagram is a file based app, meaning the diagrams you create are stored on your Mac or on iCloud, and you can copy them, rename them, or delete them outside of the *EP Diagram* app.  The diagram files you create have a file extension of /diagram/ and a specific icon and file format.  After starting the app, use the *File | Open...* menu item to open a previously saved diagram.  Use *File | Open Recent* to open a recently opened diagram.  Use the *File | New* menu item to create a new diagram. 

#+CAPTION: File dialog 
[[../gfx/file_picker.png]]

*** Dealing with file folder permissions
Apps distributed on the Apple App Store are "sandboxed", meaning they have limited access to the file folders on your computer by default.  Most file operations, such as creating, opening, and saving diagrams aren't affected by this sandboxing, but certain operations, like renaming a diagram will require that you respond to an additional dialog requesting permission to access the specific folder where the operation will take place.  Once you grant permission to access a file folder, it should not be necessary to ask for it again.
...
After calibration, if settings are set to show intervals and show conduction times, you will now see measurements on the ladder.  These will change "on the fly" as you modify the ladder.  They will remain accurate even when zooming and scrolling the image.
...
NB: In order for periods to appear in your diagram, /Show periods/ must be turned on in [[*Period settings][period settings]], and the diagram must be calibrated.  
...
*** New mark style
Newly added marks just assume the default style set in [[*Settings][settings]] but you can change this in a region to a different default.  This does not change the style of marks already present.  Use the *Style* menu item in edit mode to do that.
*** Edit label
...
** Settings
There are multiple app settings that you can set, as shown below.  Use the *EP Diagram | Settings* menu item, type *Command-,*  to open the settings window.
